...
📞 Телефонія: {phoneCount}";
            ...
            try
            {
                using (var dialog = new AddClientDialog())
                {
                    if (dialog.ShowDialog() == DialogResult.OK && dialog.CreatedClient != null)
                    {
                        ...
                        UpdateClientList();
                        MessageBox.Show("Клієнт успішно додан!", "Успіх", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            catch (Exception ex)
        ...
        {
            // Перевірка доступності списку
            if (clientListView == null)
            {
               
                MessageBox.Show("Список клієнтів недоступний.", "Помилка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // Перевірка наявності вибраних елементів
            if (clientListView.SelectedItems == null || clientListView.SelectedItems.Count == 0)
            {
            ...
            if (index < 0 || index >= clients.Count)
            {
                Debug.WriteLine($"Некоректний індекс клієнта: {index}");
                MessageBox.Show("Обраний клієнт має некоректний індекс.", "Помилка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            {
                Debug.WriteLine($"Клієнт з індексом {index} дорівнює null.");
            ...
            try
            {
                using (var dialog = new AddServiceDialog())
                {
                    ...
                    Debug.WriteLine($"Діалог завершився результатом: {result}");

                    if (result == DialogResult.OK)
                    {
                        if (dialog.CreatedService != null)
                        {
                            selectedClient.AddService(dialog.CreatedService);
                        ...
                        {
                            MessageBox.Show("Послугу не вдалося додати. Об'єкт послуги відсутній.", "Помилка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                    else
                    {
                        Debug.WriteLine("Користувач скасував додавання послуги.");
                    }
                }
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"Виняток при додаванні послуги: {ex}");
                MessageBox.Show($"Сталася помилка при додаванні послуги: {ex.Message}", "Помилка", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
